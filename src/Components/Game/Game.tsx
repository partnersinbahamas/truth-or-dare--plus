import React, { useContext, useEffect, memo, useState, useMemo, useCallback } from "react";
...
import { getDatas } from "../Helpers/Helpers";
...
export const Game = () => {
    ...
    const [modeType, setModeType] = useLocalStorage('mode', '');
    const [players, setPlayers] = useLocalStorage<Player[]>('players', []);

    const mode: ModeType | any = modes.find((el) => el.type === modeType) || null;
    ...
    const [isQuestion, setIsQuestion] = useSessionStorage('isQuestion', true);

    const [questionIndex, setQuestionIndex] = useSessionStorage('questionIndex', 0); 
    const [dareIndex, setDareIndex] = useSessionStorage('dareIndex', 0);
    const [playerIndex, setPlayerIndex] = useSessionStorage('playerIndex', 0);

    const [isChoosed, setIsChoosed] = useSessionStorage('isActionChoosed', false);
    const [isTruthDisabled, setIsTruthDisabled] = useState(false);
    const [isDareDisabled, setIsDaresDisabled] = useState(false);

    const [countActions, setCountActions] = useState(0); 
    const [countOf, setCountOf] = useState(0);

    const [isCustomActionModal, setIsCustomActionModal] = useState(false);
    const [translatedToLang, setTranslatedToLang] = useState(lang);
    ...
    const setParams = (data, index, func) => {
    ...
    useEffect(() => {
      setCountActions((current) => current + 1);
    }, [questionIndex])
